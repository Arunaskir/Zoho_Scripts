...
//
//v0.9 Initial commit
//v0.91 Step 3.5 - If Email is listed in UserFields, checks, if it does not exist in Database.
...
var aa = {
	result: undefined,
	sZohoUserEmail: 'test_user1@test.com',
...
function cbUpdateZohoUserByEmail(error, response, body) {
  ...
  if (dataResp.response.hasOwnProperty("result")) {
	...
	if (Array.isArray(jContacts)) {
	  ...
	  if (aa.aaZohoContactFields.hasOwnProperty("Email") && aa.aaZohoContactFields.Email != aa.sZohoUserEmail) {
			aaCheckIfNoDublicates(aa);
	  }
	  else {
...
function aaCheckIfNoDublicates (aa) {
  var sZohoMethod = 'searchRecords';
  var uData = '&criteria=(Email:'+aa.aaZohoContactFields.Email+')';
  var uZohoFullUrl = uZohoUrlContactsModule + sZohoMethod + uZohoToken + uZohoUrlParameters + uData;
